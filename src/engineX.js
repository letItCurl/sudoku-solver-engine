...
class Engine{
    ...
    scanAreaAndReplaceZero = function(ref, center, val){
        for(let i = center[0]-1; i < center[0]+2; i++){
            for(let j = center[1]-1; j < center[1]+2; j++){
                if(ref.grid[j][i]===0){
                    this.sudoku.grid[j][i]=val
    ...
    solveAll = function(){
        ...
        if(run===50){
            ...
            this.errors = ["//////////",'This sudoku is damned!','See the about page to understand why.', 'An assumption needs to be made or','initial input are incorrect',"//////////"]
            return false
        }else{
            return true
        }
