...
class Sudoku {
  constructor(grid) {
    ...
    _defineProperty(this, "checkInitialArea", function (x, y) {
      const area = this.getArea(x, y);
      return this.isMoreThanOne(area);
    });

    _defineProperty(this, "checkInitialRow", function (y) {
      const row = this.getRow(y);
      return this.isMoreThanOne(row);
    });

    _defineProperty(this, "checkInitialCol", function (x) {
      const col = this.getCol(x);
      return this.isMoreThanOne(col);
    });

    _defineProperty(this, "isMoreThanOne", function (arr) {
      var numbersCount = [[], [], [], [], [], [], [], [], []];
      var validated = true;
      arr.forEach(el => {
        if (el != 0) {
          numbersCount[el - 1].push(el);
        }
      });
    ...
    _defineProperty(this, "checkInput", function (x, y) {
      var msg = {
        erros: []
      };

      if (!this.checkInitialArea(x, y)) {
        msg.erros.push('doubles in the current area');
      }

      if (!this.checkInitialRow(y)) {
        msg.erros.push('doubles in the current row');
      }

      if (!this.checkInitialCol(x)) {
        msg.erros.push('doubles in the current colunm');
      }

      if (msg.erros.length === 0) {
        return true;
      } else {
        return msg;
      }
